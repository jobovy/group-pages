...
h3(#2mass). 2MASS and other catalogs

2MASS data are located at @$DATADIR/2mass@ and have been directly downloaded from the "@2MASS archive@":http://www.ipac.caltech.edu/2mass/overview/access.html. The data have been loaded into a "PostgreSQL":https://www.postgresql.org/ database @catalogs@ that is running on the server (as is usual, this database is owned by the user @postgres@). The point-sources table is @twomass_psc@. You can use it for example as 
...
Note that these queries take about five minutes to execute.
