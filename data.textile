...
The Gaia DR2 2MASS match was loaded using
<pre>> psql <notextile>-</notextile>f tmass_schema catalogs
> find tmass_best_neighbour/csv/ -name tmass_best_neighbour_*.csv.gz | xargs zcat | sed -e '/^source_id/d' | psql -c "SET TRANSACTION READ WRITE; copy gaiadr2_tmass_best_neighbour from stdin with delimiter ',' NULL as ''" catalogs</pre>

Finally, the @catalogs@ database was made read-only by default using
<pre>> psql catalogs <notextile>-</notextile>U postgres
catalogs=# set default_transaction_read_only= true;</pre>

but note that this can be turned off by simply running the last command with @false@ instead.
